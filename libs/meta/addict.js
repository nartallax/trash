...
var Addict = (() => {
	...
	var fileOfPackage = (() => {
		...
		var moduleDirName = (part, path) => {
			...
			if(dirs.length > 1) throw 'Ambiguity detected while resolving module ' + part + ': "' + dirs.join('", "') + '"'
			return dirs[0]
		}
		...
		var packageFileName = (parts, path) => {
			...
			if(paths.length > 1) throw 'Ambiguity detected while resolving package ' + name + ': "' + paths.join('", "') + '"'
	...
	var PackageStack = (() => {
		...
		PackageStack.prototype = {
			push: function(name){
				if(this.map[name]) throw 'Circular dependency: ' + this.stack.join(' -> ') + ' -> ' + name
	...
	(() => {
		...
		var fileOrThrow = name => {
			...
			if(!file) throw 'Failed to resolve ' + name;
			return file;
		}
		...
		var productOfNameOrThrow = getPackage = name => {
			...
			if(!pkg) throw 'Could not load ' + name + '. Maybe some definition mistakes?'
			return pkg;
		}
	...
	var Addict = {
		package: (name, body) => {
			if(!isValidName) throw 'Package name ' + name + ' is not valid.'
			
			name = normalizeName(name);
			
			if(name in packages) throw 'Duplicate definition of ' + name
		...
		require: (() => {
			var req = name => {
				...
				} else if(!isMainExecuting) throw 'Package ' + name + ' is required asynchronously. Such requisitions are not allowed.'
		...
		main: body => {
			if(startTimeoutHandle !== null) {
				...
				throw 'More than one entry points detected.'
			}
		...
		addRoot: (module, dir) => {
			if(!isValidName(module) && module.length > 1) throw 'Invalid module name: ' + module;
			if(sourceDirectories[dir]) throw 'Directory "' + dir + '" could not be registered as root of module ' + module + ': its already registered as root of module ' + sourceDirectories[dir];
